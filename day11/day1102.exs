    ...
    Enum.reduce(0..(length(monkeys) - 1), monkeys, fn idx, monkeys ->
      ...
      {rsent_to_true, rsent_to_false} =
        Enum.reduce(cur_monkey.items, {[], []}, fn item, {rsent_to_true, rsent_to_false} ->
          updated_item = rem(cur_monkey.op.(item), cur_monkey.lcm)
          test = cur_monkey.test

          remainder = rem(updated_item, test)
...
Enum.reduce(1..10_000, monkeys, fn _, monkeys -> MonkeyBusiness.round(monkeys) end)
